...
def matrix(): #создаём фунцию с помощью которой сгенирируем матрицу
    stolb = int(input("Введите колличество строк >>>  "))
    strok = int(input("Введите колличество столбцов >>>  "))
    return [[random.randint(1, 20) for i in range(strok)] for j in range(stolb)] #генеируем количество стобцов и строк в матрице при помощи return
b = matrix()
print(b)
